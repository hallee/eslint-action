...
const {
  GITHUB_EVENT_PATH, GITHUB_SHA, GITHUB_TOKEN, GITHUB_WORKSPACE, SOURCE_ROOT, EXTENSIONS
} = process.env;
...
function lint() {
  let enviromentExtensions = null;
  if(EXTENSIONS) {   
    try {
      JSON.parse(EXTENSIONS);
    } catch (err) {
      enviromentExtensions = null;
      console.log(`Error parsing extensions: ${err}`)
    }
  }
  const cli = new eslint.CLIEngine({
    extensions: enviromentExtensions.isArray() ? EXTENSIONS : ['.js', '.jsx', '.ts', '.tsx'],
